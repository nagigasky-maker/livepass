<?php
/**
 * ZINE Index — Issue Archive
 * Template: page-amd-zine.php
 * Slug: zine
...
<title>ZINE | ALL MUST DANCE™</title>
...
/* ZINE Index header */
.zine-index-header {
  padding:max(100px,calc(env(safe-area-inset-top)+80px)) 32px 48px;
}
.zine-index-header h1 {
...
@media (max-width:860px) {
  .zine-index-header { padding:max(80px,calc(env(safe-area-inset-top)+60px)) 24px 36px; }
  .zine-card-list { padding:0 24px; }
...
<!-- Index header -->
<div class="zine-index-header">
  <h1>ZINE</h1>
  <p>ALL MUST DANCE™ · Issue Archive</p>
</div>
